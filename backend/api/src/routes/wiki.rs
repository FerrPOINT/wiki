...
pub async fn require_wiki_auth(mut req: Request<Body>, next: Next) -> Result<Response, StatusCode> {
    let token = req
        .headers()
    ...
    req.extensions_mut().insert(WikiClaims { user_id });
...
pub async fn list_spaces() -> Json<SpaceListResponse> {
    let store = store().lock().expect("wiki store lock");
...
#[utoipa::path(
    ...
    tag = "documents",
    params(("document_id" = String, Path)),
    responses((status = 200, body = Vec<DocumentRevisionResponse>), (status = 404)),
    security(("bearer" = []))
)]
pub async fn list_document_revisions(
    Path(document_id): Path<String>,
) -> Result<Json<Vec<DocumentRevisionResponse>>, shared::AppError> {
    let store = store().lock().expect("wiki store lock");
    let id = resolve_document_id(&store, &document_id)?;
    Ok(Json(store.revisions.get(&id).cloned().unwrap_or_default()))
}
...
pub async fn list_evidence(Query(query): Query<EvidenceQuery>) -> Json<EvidenceListResponse> {
    let store = store().lock().expect("wiki store lock");
    ...
    Json(EvidenceListResponse { evidence: items })
}
