...
- Document create/view/edit/publish/archive/move flows, revision-aware backend endpoints and generated frontend API types.
- Task and phase dossiers linked to evidence and SDLC workflow context.
- Evidence registry for external links and uploaded files attached to documents, tasks or phases.
- Templates, audit log, users/settings/admin pages and global search.
- HTTP-only CLI binary `wiki` for the same public API operations as UI.
- Architecture, operations, threat model, traceability and visual screenshot evidence.

## Границы

- The repository was split from `task-tracker`; remaining legacy app/infra code is quarantined behind `legacy-tracker`.
