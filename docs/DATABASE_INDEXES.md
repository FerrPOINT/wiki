...
| `users` | `users_username_idx` | unique btree | Profile lookup |
...
| `space_members` | `space_members_space_user_idx` | unique btree | Permission lookup |
| `documents` | `documents_space_parent_slug_idx` | unique btree | Tree uniqueness |
...
- Document tree: `(space_id, parent_id, position)` and unique `(space_id, parent_id, slug)`.
