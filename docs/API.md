...
`POST /documents/{document_id}/publish` принимает `summary` и необязательный `base_revision_id`. Если `base_revision_id` передан, API сравнивает его с текущей опубликованной ревизией документа внутри publish-транзакции; несовпадение возвращает `409 CONFLICT` с сообщением `document draft is based on a stale revision`. Для первой публикации или legacy-клиентов поле можно не передавать.

`GET /documents/{document_id}/revisions` возвращает историю в порядке от последней опубликованной ревизии к первой. Endpoint bounded: без `limit` отдаёт последние 20 ревизий, `limit` ограничивается диапазоном `1..100`; необязательный `offset` (по умолчанию `0`) пропускает указанное число более новых ревизий. Для постраничного просмотра клиент запрашивает `limit=21&offset=20*N`, показывает первые 20 и использует последнюю запись как признак следующей страницы. Опубликованные ревизии immutable: новый draft или повторная публикация не меняют тело, заголовок и summary уже созданных ревизий.
...
`task-summaries` и `phase-summaries` возвращают только `space_key`, ключ, `title`, `document_count` и `evidence_count`, без вложенных документов или evidence. `limit` по умолчанию 20, допускается `1..100`; `cursor` — последний ключ предыдущей страницы, в ответе `next_cursor=null` означает конец. `total` — точное число совпадений во всём пространстве с учётом `q`, но без применения `cursor`; оно остаётся тем же даже на пустой странице после конца списка. Необязательный `q` ищет без учёта регистра по ключу, сохранённому названию и названиям связанных неархивных документов во всём пространстве до пагинации. Ключи сортируются в бинарном порядке, поэтому page boundary стабилен при совпадающих датах. Счётчик документов не включает архивные документы. Старые `/tasks` и `/phases` оставлены для совместимости, но новым каталогам следует использовать bounded summary endpoint.
...
Canonical `evidence_type` values for MVP are `external_url` and `uploaded_file`. `external_url` accepts a non-empty `url` and must not include `attachment_id` or `checksum`; `uploaded_file` accepts `attachment_id` without `url` and stores checksum from the staged attachment. Evidence can be linked to `document_id`, `task_key`, `phase_key` or their combination inside one space. If `document_id` is present and `space` is omitted, API uses the document's space; if explicit `space` conflicts with the document's space, API returns `400 VALIDATION_ERROR`. `GET /evidence` supports `space`, `document_id`, `task_key`, `phase_key`, `q`, `cursor` and `limit`; all filters and case-insensitive substring search (`q`, maximum 200 characters) run server-side within spaces visible to the caller. Results are ordered by `(created_at DESC, id DESC)`. The response contains `evidence` and nullable `next_cursor`; pass the cursor back with the same filters to read the next page. An invalid cursor returns `400 VALIDATION_ERROR`. Without `limit`, API returns the latest 30 evidence items; `limit` is clamped to `1..100`. Specific source categories such as CI job, pull request, deployment or test artifact are metadata, not separate evidence types.

## 11. Search

| Method | Path      | Назначение       |
| ------ | --------- | ---------------- |
| `GET`  | `/search` | Поиск документов и материалов |

Фильтры MVP: `q`, `space`, `task_key`, `phase_key`, `document_type`, `result_type` (`document` или `evidence`), `include_archived`, `limit`, `cursor`. Без `limit` поиск возвращает 20 результатов; `limit` ограничивается диапазоном `1..100`. `result_type` применяется до ограничения страницы, поэтому вкладка материалов не зависит от числа более новых документов. Ответ содержит `results` и `next_cursor` (`null` на последней странице). Результаты отсортированы по `(updated_at DESC, id DESC, result_type DESC)`; клиент передаёт непрозрачный `next_cursor` с теми же фильтрами для следующей страницы. Общее количество совпадений API не обещает. Ссылки на документы используют UUID, чтобы одинаковые slug в разных пространствах не вели к чужому документу.
