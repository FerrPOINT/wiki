...
| REQ-DOC-002   | Document view    | `/documents/{document_id}`                                            | `documents`, `document_revisions`                          | `/documents/:documentId`, `wiki doc get`                      | view API, draft redaction test + visual smoke |
| REQ-DOC-003   | Draft edit       | `/documents/{document_id}/draft`                                      | `document_drafts`                                          | `/documents/:documentId`, `wiki doc draft`                    | draft API, `can_edit` UI state + editor test |
| REQ-DOC-004   | Publish          | `/documents/{document_id}/publish`                                    | `document_revisions`, `document_drafts.base_revision_id`, `audit_log` | revision panel, `wiki doc publish --base-revision`            | publish flow + stale conflict tests |
...
| REQ-OPS-001   | Runtime health   | `/api/v1/health`, `/api/v1/health/ready`                        | API runtime readiness state                                | API-only deployment probes; bypass general rate limit       | health/readiness API tests      |
| REQ-AUD-001   | Audit            | `/audit-log`                                                          | `audit_log`                                                | `/audit-log`, `/admin`, `wiki audit list --limit`             | audit write/limit tests         |
| REQ-API-001   | API              | `/api/v1`, `openapi/openapi.json`, protected write `Idempotency-Key`   | DTOs + OpenAPI schemas, `idempotency_records`              | UI and CLI clients                                            | OpenAPI parity check, idempotency API tests |
