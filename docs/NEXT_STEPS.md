...
- PostgreSQL runtime persistence is behind `shared::wiki_contract::WikiBackendPort`; public Wiki DTOs, `WikiSettingsSnapshot` and the port live in `shared::wiki_contract`; the concrete SQLx adapter is private in `infra::wiki_postgres`, with connection/bootstrap, SQL constants, row mapping and all transition operation slices separated into focused modules; search and templates have the first app-level use case/repository ports, while the main Wiki route module keeps HTTP/OpenAPI responsibilities and an explicit memory test/dev backend;
...
- search q/filter/limit normalization and merge/sort/limit behavior are in `app::wiki`; template create validation/normalization is in `app::wiki`; the PostgreSQL adapter still owns SQL details behind repository ports;
...
- evidence, attachments and audit.
...
1. Introduce Wiki repository traits/use cases for identity, spaces, documents, dossiers, evidence and audit, following the search/templates slices.
