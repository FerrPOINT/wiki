...
- PostgreSQL runtime persistence is behind `shared::wiki_contract::WikiBackendPort`; public Wiki DTOs, `WikiSettingsSnapshot` and the port live in `shared::wiki_contract`; the concrete SQLx adapter is private in `infra::wiki_postgres`, with connection/bootstrap, SQL constants, row mapping, identity/auth/users/settings, spaces/space_members/tree, document/revision and task/phase dossier operations separated from the remaining operation methods, while the main Wiki route module keeps HTTP/OpenAPI responsibilities and an explicit memory test/dev backend;
...
The public API/router is now a Wiki MVP runtime with memory test fallback and SQLx/PostgreSQL persistence owned by `infra`. Identity/auth/users/settings, spaces/space_members/tree, document/revision and task/phase dossier operations have been isolated into transition adapter modules. Split the remaining operation methods into modules first, then turn the transition modules into Wiki-owned application use cases/repositories:
...
- Split the remaining evidence/attachment/template/audit/search `infra::wiki_postgres::mod` operation methods into transition modules, then into application use cases backed by Wiki repositories.
...
1. Extract evidence, attachment, template, audit and search operation methods from `infra::wiki_postgres::mod` into transition modules, then introduce app/infra repository interfaces/implementations using `domain::wiki`; the identity/auth/users/settings, spaces/space_members/tree, document/revision and task/phase dossier transition modules are already separated.
