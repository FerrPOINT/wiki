...
The public API/router is now a Wiki MVP runtime with memory test fallback and SQLx/PostgreSQL persistence owned by `infra`. Split the remaining transition adapter operation methods into Wiki-owned application use cases/repositories:
...
- Split the remaining `infra::wiki_postgres::mod` operation methods into application use cases backed by Wiki repositories.
...
1. Extract identity, spaces and document operation methods from `infra::wiki_postgres::mod` into app/infra repository interfaces/implementations using `domain::wiki`.
