...
- Latest backend WSL regression after readiness, auth session hardening, FTS query, attachment upload hardening, compose cleanup, shared tracker cleanup and sanitized revision HTML: `cargo fmt --all -- --check`, `cargo check --workspace`, `cargo test --workspace -- --test-threads=1`, `cargo clippy --workspace --all-targets -- -D warnings`.
