...
| PostgreSQL persistence | Current | `infra::wiki_postgres` owns SQLx connection/bootstrap, SQL constants, row mapping and repository implementations for users, auth sessions, spaces, members, documents, drafts, revisions, task/phase links, evidence, attachments, templates, audit and search. Published document revisions store canonical Markdown, searchable plain text and sanitized HTML rendered from Markdown. Revision history, evidence, search and audit reads are bounded before returning data. Audit writes stay in the same transaction as the mutating command and persist the API request id returned to the client. |
...
- Documents: create/get/draft/publish/archive/move, immutable revision detail and bounded latest-first revision history; archived documents and documents inside archived spaces reject write commands, including new evidence attached to archived pages.
...
- Latest backend WSL regression after readiness, auth session hardening, FTS query, attachment upload hardening, compose cleanup, shared tracker cleanup, sanitized revision HTML, public `body_html` API exposure, archived-document evidence rejection, request-id audit propagation and bounded revision/evidence/search/audit reads: `cargo fmt --all -- --check`, `cargo check --workspace`, `cargo test --workspace -- --test-threads=1`, `cargo clippy --workspace --all-targets -- -D warnings`.
- Focused PostgreSQL test group passes against an isolated temporary WSL PostgreSQL database through `pwsh -File scripts/postgres-smoke-wsl.ps1`; the same test group also compiles and safely skips without `WIKI_TEST_DATABASE_URL`. Latest run covered 10 `wiki_postgres_` tests including request-id audit persistence with bounded audit reads, bounded revision history after router/backend rebuild, refresh rotation, logout invalidation for access/refresh tokens, viewer/editor/space-admin/global-admin permission boundaries, archived document and archived space draft/publish/move/archive/evidence-create/task-link/phase-link rejection plus staged attachment reuse and owner mismatch rejection.
...
- Latest frontend regression after attachment metadata/download, search filter UI, task/phase document link UI, evidence deep-link detail, production form cleanup, audit cache invalidation, bounded revision/evidence/search/audit list reads, audit request-id display, task/phase space-context handling and API request-id headers: `npm run typecheck`, `npm run test`, `npm run lint`, `npm run format:check`, `npm run build`, `npm run test:e2e -- --project=chromium`.
...
- Screenshot script passed against `vite preview`: `npm run shoot:evidence` captured 17 desktop and 5 mobile MVP screenshots, including an empty production login form without demo credentials and the task/phase space selector states.
