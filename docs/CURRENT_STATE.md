...
| Runtime composition | Current | API/server state uses `app::WikiAppContext` with runtime config only. Docker Compose and init/deploy scripts contain backend, frontend, PostgreSQL and an uploads volume; extra cache/worker/migrator services are not part of the base Wiki runtime. Compose passes `WIKI_ENVIRONMENT` and `WIKI_SERVER__CORS_ALLOWED_ORIGINS` into the backend, and `WIKI_ENVIRONMENT=production` rejects wildcard CORS, non-HTTPS origins, origins with path/query parts, empty PostgreSQL URL, short JWT secrets and insecure refresh cookies before startup proceeds. `scripts/deploy-production.sh` also refuses to run unless `WIKI_ENVIRONMENT=production` is set in the shell environment or `.env`. The backend Docker image runs as the non-root `wiki` user (`10001:10001`) with writable uploads ownership. The server can run a bounded in-process maintenance loop for expired staged uploads and expired idempotency replay rows. The real Wiki server does not construct task-tracker service graphs. |
...
| Screenshot evidence | Current | 17 desktop screenshots exist for the MVP page set; `docs/assets/screens/manifest.md` references the complete inventory. The root README shows only meaningful product surfaces — auth chrome (login/register) and mobile viewports are not part of README evidence. |
...
- Latest backend WSL regression after readiness, auth session hardening including admin deactivation revocation, FTS query, attachment upload hardening, compose cleanup, shared tracker cleanup, sanitized revision HTML, public `body_html` API exposure, archived-document evidence rejection, stale publish conflict, request-id audit propagation, idempotent write replay coverage, security-header and production-config regression coverage, `/metrics` Prometheus HTTP plus Wiki product/security counters and bounded revision/evidence/search/audit reads: `cargo fmt --all -- --check`, `cargo check --workspace`, `cargo test --workspace -- --test-threads=1`, `cargo clippy --workspace --all-targets -- -D warnings`.
- Focused PostgreSQL test group passes against an isolated temporary WSL PostgreSQL database through `pwsh -File scripts/postgres-smoke-wsl.ps1`; the same test group also compiles and safely skips without `WIKI_TEST_DATABASE_URL`. Latest run covered 13 `wiki_postgres_` tests including request-id audit persistence with bounded audit reads, idempotent write replay without duplicate evidence/audit rows, maintenance cleanup for expired staged attachments and expired idempotency rows, stale `base_revision_id` publish rejection, bounded revision history after router/backend rebuild, refresh rotation, logout invalidation and admin deactivation revocation for access/refresh tokens, viewer/editor/space-admin/global-admin permission boundaries, archived document and archived space draft/publish/move/archive/evidence-create/task-link/phase-link rejection plus staged attachment reuse and owner mismatch rejection.
...
- Screenshot script passed against `vite preview`: `pnpm shoot:evidence` captured 17 desktop MVP screenshots including the task/phase space selector states.
