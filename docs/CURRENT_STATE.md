...
- Latest backend WSL regression after readiness, auth session hardening, FTS query, attachment upload hardening, compose cleanup, shared tracker cleanup, sanitized revision HTML, public `body_html` API exposure, archived-document evidence rejection and request-id response propagation: `cargo fmt --all -- --check`, `cargo check --workspace`, `cargo test --workspace -- --test-threads=1`, `cargo clippy --workspace --all-targets -- -D warnings`.
...
- Latest frontend regression after attachment metadata/download, search filter UI, task/phase document link UI, evidence deep-link detail, production form cleanup, audit cache invalidation, task/phase space-context handling and API request-id headers: `npm run typecheck`, `npm run test`, `npm run lint`, `npm run format:check`, `npm run build`, `npm run test:e2e -- --project=chromium`.
