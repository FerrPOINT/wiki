...
| CLI shape                  | Current | `wiki` CLI command surface drafted for public API operations; 12 mocked HTTP smoke tests cover auth, spaces, documents, task/phase dossiers, templates, search, URL/file evidence request flows and simple/structured API error envelopes through the public API                                                                                |
...
- `wsl bash -lc 'cargo test --manifest-path backend/Cargo.toml -p wiki-cli -- --nocapture'` passed: 12 mocked HTTP smoke tests cover auth, spaces, document get/create/draft/publish/archive/move/history, task/phase dossier get/docs/evidence/link-doc, template list/apply, filtered search, URL/file evidence request flows and simple/structured API error envelopes.
...
- CLI error envelope check passed: CLI renders current simple `{"error":"..."}` responses and target structured `{"error":{"code","message","requestId","details"}}` responses as concise command errors.
