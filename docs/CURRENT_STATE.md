...
- Latest frontend regression after attachment metadata/download, search filter UI, task/phase document link UI, evidence deep-link detail, production login cleanup and audit cache invalidation: `npm run typecheck`, `npm run test`, `npm run lint`, `npm run format:check`, `npm run build`, `npm run test:e2e -- --project=chromium`.
