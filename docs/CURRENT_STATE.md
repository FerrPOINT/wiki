...
| Wiki app helpers | Current | Shared Wiki normalization, role/access predicates, Markdown text extraction, checksums, safe download names, password hashing and Wiki JWT/session token helpers live in `app::wiki` instead of private API route helpers |
...
- `wsl bash -lc 'cargo test --manifest-path backend/Cargo.toml -p app wiki_auth -- --nocapture'` passed after moving Wiki password hashing, token hashing and JWT helpers from API routes to `app::wiki`.
- `wsl bash -lc 'cargo check --manifest-path backend/Cargo.toml -p app -p api -p server'` passed after the Wiki auth helper extraction.
- `wsl bash -lc 'cargo test --manifest-path backend/Cargo.toml --workspace -- --test-threads=1 --nocapture'` passed after the Wiki auth helper extraction; API PostgreSQL smoke still skips when `WIKI_TEST_DATABASE_URL` is unset in WSL and infra Docker repository tests remain ignored.
...
- `wsl bash -lc 'cargo clippy --manifest-path backend/Cargo.toml -p app -p api --all-targets -- -D warnings'` passed after the Wiki auth helper extraction.
...
- `vite build` passed.
- Host `npm run typecheck`, `npm run test`, `npm run lint`, `npm run format:check`, `npm run build` passed after the Wiki auth helper extraction; `pnpm` itself remains blocked by the local Corepack error noted below.
