...
| CLI shape                  | Current | `wiki` CLI command surface drafted for public API operations; mocked HTTP smoke tests cover filtered search including `document_type`, document create, template apply by `document_type` and file-evidence upload/claim request flow                                                                                                    |
