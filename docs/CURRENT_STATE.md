...
| CLI shape                  | Current | `wiki` CLI command surface drafted for public API operations; 11 mocked HTTP smoke tests cover auth, spaces, documents, task/phase dossiers, templates, search and URL/file evidence request flows through the public API                                                                                                                 |
...
- `wsl bash -lc 'cargo test --manifest-path backend/Cargo.toml -p wiki-cli -- --nocapture'` passed: 11 mocked HTTP smoke tests cover auth, spaces, document get/create/draft/publish/archive/move/history, task/phase dossier get/docs/evidence/link-doc, template list/apply, filtered search and URL/file evidence request flows.
