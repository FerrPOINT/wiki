...
| CLI | Current | `wiki` CLI covers the MVP public API groups for auth, users, spaces/member management, documents/revisions, task/phase dossiers, evidence, attachments, templates, audit, search and settings. Every CLI HTTP request sends `X-Request-ID`; write commands also send `Idempotency-Key`. |
...
- Audit: append-only write action trail for core MVP commands, including request id correlation for API/UI/CLI troubleshooting.

## Verified Checks

- Latest backend WSL regression after readiness, auth session hardening, FTS query, attachment upload hardening, compose cleanup, shared tracker cleanup, sanitized revision HTML, public `body_html` API exposure, archived-document evidence rejection and request-id audit propagation: `cargo fmt --all -- --check`, `cargo check --workspace`, `cargo test --workspace -- --test-threads=1`, `cargo clippy --workspace --all-targets -- -D warnings`.
- Focused PostgreSQL test group passes against an isolated temporary WSL PostgreSQL database through `pwsh -File scripts/postgres-smoke-wsl.ps1`; the same test group also compiles and safely skips without `WIKI_TEST_DATABASE_URL`. Latest run covered 10 `wiki_postgres_` tests including request-id audit persistence, refresh rotation, logout invalidation for access/refresh tokens, viewer/editor/space-admin/global-admin permission boundaries, archived document and archived space draft/publish/move/archive/evidence-create/task-link/phase-link rejection plus staged attachment reuse and owner mismatch rejection.
...
- Latest frontend regression after attachment metadata/download, search filter UI, task/phase document link UI, evidence deep-link detail, production form cleanup, audit cache invalidation, audit request-id display, task/phase space-context handling and API request-id headers: `npm run typecheck`, `npm run test`, `npm run lint`, `npm run format:check`, `npm run build`, `npm run test:e2e -- --project=chromium`.
