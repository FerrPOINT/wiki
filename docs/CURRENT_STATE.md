...
| CLI shape                  | Current | `wiki` CLI command surface drafted for public API operations; 7 mocked HTTP smoke tests cover filtered search including `document_type`, document create, document draft/publish/archive/move/history, template apply by `document_type`, URL/file evidence creation with document/task/phase owners and evidence list owner filters               |
...
- `wsl bash -lc 'cargo test --manifest-path backend/Cargo.toml -p wiki-cli -- --nocapture'` passed: 7 mocked HTTP smoke tests cover filtered search, document create, document draft/publish/archive/move/history, template apply and URL/file evidence request flows.
