...
| PostgreSQL smoke | Current | Env-gated tests with the `wiki_postgres_` prefix cover disabled registration, archived document write rejection, membership revocation, persistence across backend/router rebuilds and FTS index-plan evidence. `scripts/postgres-smoke.ps1` runs them against Docker Postgres; `scripts/postgres-smoke-wsl.ps1` runs the same suite against an isolated temporary WSL PostgreSQL database. |
