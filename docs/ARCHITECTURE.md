...
| Разбиение transition SQLx adapter на app/repositories               | В работе: transition adapter разложен на focused modules; users/settings, search, templates и pool-backed audit list/write вынесены первыми вертикальными срезами в app-level use cases + repository ports; следующий шаг - такие же use cases и dedicated repositories для auth/session/current-user, spaces, documents, dossiers и evidence; transactional audit helper остаётся рядом с SQLx write-транзакциями до выноса соответствующих write-use-case                                                                                                                                                                                                        |
