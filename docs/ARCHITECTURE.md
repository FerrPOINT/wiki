...
| Route-level SQLx runtime persistence                                | Готово для MVP: PostgreSQL adapter закрыт за `shared::wiki_contract::WikiBackendPort` и вынесен из API crate в `infra::wiki_postgres`; `connection`, SQL constants и row mapping отделены от operation methods; базовые space-role checks включены; attachment bytes вынесены за Wiki storage port; shared Wiki validation/auth/search helpers вынесены в app layer |
...
| Разбиение transition SQLx adapter на app/repositories               | Следующий шаг                                                                                                                                                                                                                                                                                                                                                       |
