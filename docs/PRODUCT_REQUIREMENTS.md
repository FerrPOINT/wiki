...
> Платформенная identity-модель: Central Auth является единственным источником
> пользователей и браузерных сессий. Wiki показывает read-only каталог для
> назначения, создаёт локальную проекцию строго по `sub` и не предоставляет
> локальное создание учёток или назначение пользовательских ролей.
...
- центральные пользователи с историческими локальными профилями для совместимости;
- spaces;
...
## 4. Доступ и исторические роли

В интегрированном режиме любой вошедший пользователь может работать со всеми
пространствами. Локальное членство не определяет доступ и не назначается через UI.
Центральные личные API-токены ограничены сервисными `read/write` scopes.
Ниже сохранена историческая модель standalone/API-клиентов; её данные не
удаляются этим изменением.
...
потери локального текста и предлагает повторить публикацию после проверки.

Администратор может пройти всю append-only историю аудита постранично. Счётчики на экране относятся только к текущей странице и не выдаются за общие итоги; лимит страницы показан явно. Новые события не сдвигают границы уже открытых страниц.

Архивация пространства в UI требует явного подтверждения с названием и последствиями: документы, материалы и связи остаются доступны для чтения, но их изменение блокируется. Отмена не отправляет запрос; ошибка остаётся в диалоге для повторной попытки. После успеха пространство и его архивный статус остаются видимыми в списке.
...
- Повтор successful protected domain/admin write с тем же `Idempotency-Key` возвращает сохранённый ответ без повторного domain write и audit write.
