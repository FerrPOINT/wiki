...
| `/users`                 | Пользователи       | Смотреть профили Wiki и перейти к управлению в Admin Panel   |
...
| ----- | ------- | --------------- | --------------- | ------------------- |
| `/login` | Вход в Wiki | validation error, auth error, loading | submit credentials | — |
| `/register` | Регистрация, когда она включена на инстансе | disabled registration, validation error, loading | create account | — |
| `/` | Обзор текущего Wiki состояния | loading, empty lists, API error | open create/search/dossier links | `03-dashboard.png` |
| `/spaces` | Spaces, document tree and access entry | loading, empty spaces, tree error, member permission error | open tree/document, create/update/archive space, assign/remove members when allowed | `04-spaces.png` |
...
| `/documents/:documentId` | Read/edit/publish document | loading, not found, permission error, archive write blocked | save draft, publish, archive, move, open revision | `06-document-view.png` |
| `/tasks` | List known task dossiers | loading, empty tasks, API error | open task key | `07-task-dossiers.png` |
| `/tasks/:taskKey` | Task knowledge dossier | loading, not found, empty docs/evidence | open document/evidence/phase links | `08-task-dossier-detail.png` |
...
| `/users` | Read-only Central Auth profile directory | loading, empty/no-match, permission error, dense directory | search, paginate, open Admin Panel accounts | `14-users.png` |
| `/settings` | Safe runtime snapshot | loading, admin error, missing config values | reload settings | `15-settings.png` |
| `/search` | Global Wiki search | empty query, empty results, loading, error | query, filter by space/type/task/phase | `16-search.png` |
